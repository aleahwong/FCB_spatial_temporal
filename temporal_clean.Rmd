...
10. Graphing weighted FCB scores for each taxa (Line 1282)
11. Correlations of FCB in species (Line 1387)
12. Percent contribution to indices (Line 1423)
13. 
...
###STEP 12: Percent contribution to index: % contribution of taxonomic groups relative to F, C, B relative to the proportion of total production ###
```{r}
...
NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Chapter_2/Data/yearly_production_percentage_bytaxa.csv")
write.csv(totalproduction ,file=NAME)
```

```{r Load in data}
#Run code chunk 1 ("Loading in data") from Step 1, line ~1282 and these lines
...
```{r Percent contribution of each taxa to F}
...
plot <- ggplot(prod_index, aes(x=Year, y=as.numeric(ratio) , color=species)) + geom_point()+
      ...
      labs(y = "Contribution to F index:contribution to production")
      # ylim(0, 1.5)

#SAVE
# setwd("/Users/aleahw/Documents/Master's!!/Chapter_2/Figures")
# ggsave("Ratio_F_prod9-6.png", plot, height = 4, width = 8, dpi = 600) #make sure wd is set
```

```{r Percent contribution of each taxa to C}
#merge
...
plot <- ggplot(prod_index, aes(x=Year, y=as.numeric(ratio) , color=species)) + geom_point()+
      geom_hline(yintercept = 1)+
      labs(y = "Contribution to C index:contribution to production")
      # ylim(0, 1.5)

#SAVE
# ggsave("Ratio_C_prod9-6.png", plot, height = 4, width = 8, dpi = 600) #make sure wd is set
```

```{r Percent contribution of each taxa to B}
#merge
...
#SAVE
# ggsave("Ratio_B_prod9-6.png", plot, height = 4, width = 8, dpi = 600) #make sure wd is set
```
...
weighting_totals2$species <- factor(weighting_totals2$category, levels=c('algae','crustacean', 'mollusc','finfish'))
