...
F_weighted_country <- read.csv("/Users/aleahw/Documents/Master's!!/Chapter_2/Data/Fweighted_bycountry926.csv")
F_weighted_country <- F_weighted_country[, -c(1)]
C_weighted_country <- read.csv("/Users/aleahw/Documents/Master's!!/Chapter_2/Data/Cweighted_bycountry926.csv")
C_weighted_country <- C_weighted_country[, -c(1)]
B_weighted_country <- read.csv("/Users/aleahw/Documents/Master's!!/Chapter_2/Data/Bweighted_bycountry926.csv")
...
Egypt <- subset(FCB_countries, Country =="Egypt")
```
...
plot <- ggplot(heatmapdata, aes(y= index, x = Region, fill= score)) + 
  ...
  scale_y_discrete(limits = c('Biodiversity', 'Climate', 'Food'), labels = c("B", "C", "F"))+
   scale_x_discrete(limits = c('Chile', 'China','Egypt', 'Nigeria','Norway', 'Africa', 'Americas', 'Asia','Europe','Oceania'), labels = c('Chile', 'China','Egypt','Nigeria','Norway', 'Africa', 'Americas', 'Asia','Europe','Oceania'))
...
SD_F_facet <- cbind(now_then_meansF, country_now_then_meansF[, c(3:6)]) %>%
...
SD_C_facet <- cbind(now_then_meansC, country_now_then_meansC[, c(3:6)]) %>%
  pivot_longer(cols = c(!Year), names_to = "Region", values_to = "Mean") %>%
    ggplot(aes(x= Year, y=Mean, color = Year))+
    geom_boxplot()+
    facet_wrap(~Region)+
...
SD_B_facet <- cbind(now_then_meansB, country_now_then_meansB[, c(3:6)]) %>%
  pivot_longer(cols = c(!Year), names_to = "Region", values_to = "Mean") %>%
    ggplot(aes(x= Year, y=Mean, color = Year))+
    geom_boxplot()+
    facet_wrap(~Region)+
...
country_case_study <- allprod[allprod$Country.Name.En %in% c("Norway", "Chile", "Nigeria", "Egypt"), ]
...
highestC2 <- highestC2[c(1:4), c(1, 3, 5)] 

highestB2 <- country_case_study %>%
  group_by(Country.Name.En) %>%
  filter(b == max(b))

highestB2 <- highestB2[c(1:4), c(1, 3, 6)] 
