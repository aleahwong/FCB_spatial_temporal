...
final_output <- read.csv("/Users/aleahw/Documents/Master's!!/Chapter_2/Rerun_Aug2025/scores_weightedbytaxa.csv")
...
# Build summary table
avg_changes <- tibble(
  Scenario = c("Low edible weight", "Live weight", "High edible weight"),
...
# ggsave("contributiontoindex.png", fig2, width = 10, height = 6, dpi = 300)
```

#graphing production of fisheries and aquaculture over time
```{r}
...
prod <- read.csv("/Users/aleahw/Documents/Master's!!/Chapter_2/Rerun_Aug2025/total_capture_aqua_prod.csv")

# Name year columns correctly (columns 4 to 77)
  colnames(prod)[2:75] <- as.character(1950:2023)
...
plot <- ggplot(data_long, aes(x = year, y = production, color = source)) +
  geom_line(size = 1.2) +
  
  # Highlight ranges
  annotate("rect", xmin = 1976, xmax = 1980, ymin = -Inf, ymax = Inf,
  ...
  annotate("text", x = 1978, y = max(data_long$production) * 1.05,
           label = "1976–1980", size = 8, fontface = "bold", color = "skyblue4") +
  annotate("text", x = 2021, y = max(data_long$production) * 1.05,
           label = "2019–2023", size = 8, fontface = "bold", color = "orange4") +
  
  labs(color = "", y = "Production, millions of tons (Live weight)") +
  scale_y_continuous(
    labels = function(x) x / 1e6)+   # divide all y-axis ticks by 1,000,000
...
# 
# plot <-
#   ggplot(data_long, aes(x = year, y = production, color = source))+
#   geom_line(size = 1.2)+
#   labs(color = "", y = "Tons (Live weight)")+
#   theme_classic()+
...
setwd("/Users/aleahw/Documents/Master's!!/Chapter_2/Rerun_Aug2025")
# ggsave("prod_time.png", plot, height = 8, width = 18, dpi = 800) #make sure wd is set
